"""
1. Process the raw model for convex decomposition with V-HACD 
2. Write the URDF file.

V-HACD repo: https://github.com/kmammou/v-hacd
Clone the repo and compile the package
...
def run_vhacd(vhacd_dir,
    ...
    cmd = "cd %s && %s --input %s --output %s --log %s --resolution %s --depth %s --concavity %s --planeDownsampling %s --convexhullDownsampling %s --alpha %s --beta %s --gamma %s \
        --pca %s --mode %s --maxNumVerticesPerCH %s --minVolumePerCH %s --convexhullApproximation %s --oclDeviceID %s" % (
        vhacd_dir, vhacd_executable, input_file, output_file, log, resolution,
        depth, concavity, planeDownsampling, convexhullDownsampling, alpha,
        beta, gamma, pca, mode, maxNumVerticesPerCH, minVolumePerCH,
        convexhullApproximation, oclDeviceID)
...
if __name__ == '__main__':
    ...
    object_name = "Gotham_pan_pour_pca"
    data_dir = os.path.join(data_root_dir, object_name)

    vhacd_dir = os.path.join(root_dir, 'processing')
    input_file = os.path.join(data_dir, object_name + '_mesh_0.obj')
    output_file = os.path.join(data_dir, object_name + '_mesh_0_vhacd.obj')
    run_vhacd(vhacd_dir, input_file, output_file)

    urdf_path = os.path.join(data_dir, object_name + '_mesh_0.urdf')
    obj_original_file = object_name + '_mesh_0.obj'
    obj_vhacd_file = object_name + '_mesh_0_vhacd.obj'

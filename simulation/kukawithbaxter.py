...
class KukaXBaxtor:
  ...
  def __init__(self, rootPath=None, timeStep=0.01):
    ...
    self.jointID = {'lbr_iiwa_joint_1': 0,
                    'lbr_iiwa_joint_2': 1,
                    'lbr_iiwa_joint_3': 2,
                    'lbr_iiwa_joint_4': 3,
                    'lbr_iiwa_joint_5': 4,
                    'lbr_iiwa_joint_6': 5,
                    'lbr_iiwa_joint_7': 6,
                    'left_gripper_base': 7, # fixed
                    'left_endpoint': 8, # fixed
                    'l_gripper_l_finger_joint': 9, # prismatic
                    'l_gripper_l_finger_tip_joint': 10, # fixed
                    'l_gripper_r_finger_joint': 11, # prismatic
                    'l_gripper_r_finger_tip_joint': 12} # fixed


  def reset(self):
    p.setAdditionalSearchPath(pybullet_data.getDataPath())

    self.kukaUid = p.loadURDF(os.path.join(self.rootPath, "baxter_common/kuka_with_baxtergripper.urdf"),
                              useFixedBase=True)
  ...
  # TODO: add endeffector orientation to the applyAction
  def applyAction(self, motorCommands):
    ...
    if (self.useInverseKinematics):
      ...
      orn = p.getQuaternionFromEuler([0, -math.pi, math.pi/2])  # -math.pi,yaw])

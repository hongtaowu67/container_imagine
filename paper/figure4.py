...
################################## Figure 4b
                # Load pour_nominal
                p.loadURDF(self.content_urdf, basePosition=self.pour_pos_nominal, useFixedBase=True)
                # Load cup1
                self.cup_id1 = p.loadURDF(self.cup_urdf)
                p.changeDynamics(self.cup_id1, -1, mass=1)

                p.setCollisionFilterPair(self.cup_id, self.cup_id1, -1, -1, 0)

                p.resetBasePositionAndOrientation(self.cup_id1,
                                                posObj=self.cup_pos,
                                                ornObj=p.getQuaternionFromEuler(self.cup_initial_orn))
                ...
                # parentFrameOrientation: the joint frame orn in the object frame
                # childFrameOrientation: the joint frame orn in the world frame if the child frame is set to be -1 (base)
                cup_constraint_Id1 = p.createConstraint(self.cup_id1, -1, -1, -1, p.JOINT_FIXED, jointAxis=[0, 0, 0],
                                                        parentFramePosition=[cup_half_length, 0.0, self.pour_pos[2]-self.cup_pos[2]], 
                                                        childFramePosition=self.pour_pos,
                                                        parentFrameOrientation=p.getQuaternionFromEuler([0, 0, 0]),
                                                        childFrameOrientation=p.getQuaternionFromEuler(self.cup_initial_orn))
                ...
                for i in range(self.pour_simulation_iteration):
                    ...
                    orn = p.getQuaternionFromEuler([0, np.pi/6 * math.sin(math.pi * 2 * i / int(4 * self.pour_simulation_iteration)), planar_angle])
                    p.changeConstraint(cup_constraint_Id1, pivot, jointChildFrameOrientation=orn, maxForce=50)

                print("Finish loading cup1...")

                # Load cup2
                self.cup_id2 = p.loadURDF(self.cup_urdf)
                p.changeDynamics(self.cup_id2, -1, mass=1)

                p.setCollisionFilterPair(self.cup_id, self.cup_id2, -1, -1, 0)
                p.setCollisionFilterPair(self.cup_id1, self.cup_id2, -1, -1, 0)

                p.resetBasePositionAndOrientation(self.cup_id2,
                                                posObj=self.cup_pos,
                                                ornObj=p.getQuaternionFromEuler(self.cup_initial_orn))
                ...
                # parentFrameOrientation: the joint frame orn in the object frame
                # childFrameOrientation: the joint frame orn in the world frame if the child frame is set to be -1 (base)
                cup_constraint_Id2 = p.createConstraint(self.cup_id2, -1, -1, -1, p.JOINT_FIXED, jointAxis=[0, 0, 0],
                                                        parentFramePosition=[cup_half_length, 0.0, self.pour_pos[2]-self.cup_pos[2]], 
                                                        childFramePosition=self.pour_pos,
                                                        parentFrameOrientation=p.getQuaternionFromEuler([0, 0, 0]),
                                                        childFrameOrientation=p.getQuaternionFromEuler(self.cup_initial_orn))
                ...
                for i in range(self.pour_simulation_iteration):
                    ...
                    p.changeConstraint(cup_constraint_Id2, pivot, jointChildFrameOrientation=orn, maxForce=50)
...
if __name__ == "__main__":
    obj_urdf = "/home/hongtao/Dropbox/ICRA2021/data/Ikea_Nojsam_Basket_Red_pour_pca/Ikea_Nojsam_Basket_Red_pour_pca_mesh_0.urdf"
    ...
    pour_pos = np.array([-0.1875914390193092, -0.3162307304073783, 0.21186800646781934])
